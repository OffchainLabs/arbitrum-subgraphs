...
describe("Message events", () => {
  ...
  describe("MessageReceived", () => {
    ...
    ].forEach(({ network, sourceDomain }) => {
      test(`MessageReceived created and stored (${network})`, () => {
  ...
  describe("MessageSent", () => {
    [
    ...
    ].forEach(({ network, sourceDomain, destinationDomain }) => {
      test(`MessageSent created and stored (${network})`, () => {
        ...
        const sender = Address.fromString(
          "9481EF9E2CA814FC94676DEA3E8C3097B06B3A33"
        );
        // MessageSent from Avalanche is ignored
        const handler =
          network === "L1" ? handleMessageSentL1 : handleMessageSentL2;
      ...
      test(`Older MessageSent event with same (nonce, sourceDomain) is skipped (${network})`, () => {
        const newMessageSentToValidNetwork = createMessageSentEvent(
          Bytes.fromHexString(
      ...
      test(`Newer MessageSent event with same (nonce, sourceDomain) replaces the old one (${network})`, () => {
        const newMessageSentToValidNetwork = createMessageSentEvent(
          Bytes.fromHexString(
            `000000000000000${sourceDomain}0000000${destinationDomain}`
          )
